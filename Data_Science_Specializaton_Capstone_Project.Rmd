...
- Using the elaborateed insights and iterating the think-try-think cycle we can subsequently 
...
- The word set of the text corpus is expected to mainly correspond to the standard english one.
...
Aiming a non-data scientist manager with this report, all used processing and configuration routines and  stay 
...
Data loading and unpacking is performed hidden using R function *utils::download.file* and *utils:unzip* . 
...
```{r Building train corpus,echo = F, cache=T}
...
```{r Building reference word lists,echo=FALSE, cache=T, split= F}
if (!dir.exists(file.path(tmpdir,"before"))) {
        dir.create(file.path(tmpdir,"before"),recursive = T)
        dir.create(file.path(tmpdir,"after"),recursive = T)
}
...
word variants list (total of `r  nrow(dt_words_EN)` words and variants from british, american, canadian and other flavors 
...
```{r Building tidy data,echo=FALSE, cache=TRUE}
...
The word count in the data set changed as follows:  

----------------------------------------------  
Stage                     |   Number of words  
----------------------------------------------  
Raw data set              |  `r cntinitial`  
Non-latin removed         |  `r cntlatin`  
Verified english words    |  `r cnteng`  
----------------------------------------------  

`round(cntinitial-cntlatin)` or (`r round((cntinitial-cntlatin)/cntinitial*100,3)`%) words were 
identified and removed as containing the non-latin characters.  
...
The ward "and" stays away from other popular words, showing less binding to the rest. The words from the Top-20 build 
own clusters. 
...
(`r length(wordFreqDescNorm)*(length(wordFreqDescNorm)-1)`)!  Statistically proven, a language is not a simply word mess.
The top-20 two-word-combinations are (absolute numbers here):

```{r Bigrams, echo=F, comment=''}
...
```{r Top-20 four-grams, echo=F}
for (i in 1:20) print(paste(names(four_grams[i]),four_grams[i],sep = " - "))
```

At this place we have at last a surprise of some kind. The number of the unique word combinations grows from three to four 
words only minuscule (from `r length(trigrams)` to  `r length(four_grams)`) compared to the possible permutations for both 
...
Taken the known ranked frequencies of n-grams, we could try build the prediction model at this basis.  Loving extremes, we take 
...
At this basis some kind of "saturation curve" was plotted, reflecting how many tries we need to reach the given coverage 
level for summarized frequency of related outcomes.
...
- with predictor of two words needs only 50 tries
...
2. All the technical routines for selection of the lists are as described above.
...
2. The next omnipresent controversy is that between the statistical performance of the model and hardware performance.
...
expression `r names(bigrams)[1]` to the user.
4. The limitations put by hardware and user interfaces of the gadgets like smartphone reinforce the considerations above.
5. We need for development much enough ressources, to produce  the parsimonious, performant and efficient end product. 
...
#rm(dummies, used_words)
#invisible(gc())

dt_words_EN <- data.table(freq = c(used_words), words_EN = c(names(used_words),names(rest_words)))
setkey(dt_words_EN, words_EN)
...
predictEngine <- function(xpr) {
        ...
        xprOrig <- xpr # preserving the original search expression
        ...
        if (Encoding(xpr) == "unknown") {  #we don't deal with the foreign and mixed inputs
                ...
                askDB <- function(xpr) {
                        ### Conditioning predictor expression
                ...
                if (xpr == "") {
                        ...
                        if (nrow(rtrn) == 0 ) { 
                                ...
                                xpr <- xprOrig
                                lgthOrig <- stri_count(xprOrig, fixed = " ")+1
                                ...
                                while (lgth > 1 && lgth == lgthOrig && nrow(rtrn) == 0) {
                                        xpr <-gsub(".$","",xpr)
                                        rtrn <- askDB(xpr)
                                        lgth <- stri_count(xpr, fixed = " ",case_insensitive = T)+1
                                } 
                                rm(xpr)
                        ...
                        if (nrow(rtrn) == 0) {
                                ...
                                xpr <- xprOrig #beginnig from scratch
...
inWordPredictEngine <- function(xpr) {
        ...
        if (stri_length(xpr) > 0) {
                ...
                if (nrow(rtrn) == 0) {
                        ...
                        if (nrow(rtrn) == 0){
                                rtrn <- head(arrange(dt_words_EN, desc(freq)),limit) # Yes, we can! Even, if these are the trivials.
                        }
                }
        }
        else {
...
```{r Testing the prediciton prototype, echo=T, collapse = T}
...
head(rslt)
# Normal case, predictor not found - shortening the second word
system.time(rslt <- predictEngine("red readd"))
head(rslt)
# Control for the second part
system.time(rslt <- predictEngine("readdd"))
head(rslt)
# Predictor ends with punctuation char - nothing to be returned
system.time(rslt <- predictEngine("red river ,"))
head(rslt)

# After foreign words not suggestions delivered
...
- We have succesfully downloaded and performed an exploration data analysis of the english recommenden text corpus.  
- The resulsts of the analysis let us build a plain, but performant prediction algorithm.  
- The insights, algorithm and prototype of prediction engine are a very good basis for further technically perfromant and 
parsimonious development.
