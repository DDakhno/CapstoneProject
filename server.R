    ...
    function(input, output, session) {
        ...
        observe({
            if(!is.null(input$inSelect)){
                if (!grepl("[[:blank:]]+$",input$inSelect)) {
                    # Exchanging the last incomplete word through one of proposals
                    z <- paste(gsub("[[:blank:]]+[^[:blank:]]+$"," ",input$userInput),input$inSelect, collapse=" ")
                } else {
                    # Adding one of proposals to the input line
                    z <- paste(input$userInput,input$inSelect, collapse=" ")
                }
        ...
        observe({
             if(!is.null(input$userInput) && stri_length(input$userInput) > 0)   {
                 updateSelectInput(session, 'inSelect', choices = c({ predictionEngine(input$userInput) }) )
             }
        })
        ...
        #         while(!is.null(yy())) {
        #             if (grepl("[[:blank:]]+$",y)) {
        #                 z <- paste(gsub("[[:blank:]]+[^[:blank:]]+$"," ",x),y, collapse=" ")
        #             } else {
        #                 z <- paste(x,y, collapse=" ")
        #             }
        #             y <- NULL
        ...
        #     } else {
        #         if (stri_length(x) > 0) {
        #             otc <- predictionEngine(x)$outcome
        #             if (is.null(otc))
        #                 otc <- character(0)
        # 
...
#     y <- input$inSelect
#     if( !is.null(y) &&  grepl("[[:blank:]]$",y)){
#         z <- paste(input$userInput,y, collapse=" ")
#     } else {
#         z <- paste(gsub("[[:blank:]]+[^[:blank:]]+$"," ",input$userInput),y, collapse=" ")
#     }
#     updateTextInput(session,'userInput',value = z)
#     fromUpdate <- TRUE
# })
